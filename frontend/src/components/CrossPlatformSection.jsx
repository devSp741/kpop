"use client";

import React from "react";
import Link from "next/link";
import { motion } from "framer-motion";

const heroIdolsImg = "/assets/hero-idols-DZmJP1G3.jpg";
const idolFallback = "/assets/idol-2-CpsI_gBv.jpg";

// Platform SVG Icons for Badges
const InstagramIcon = ({ className = "w-3.5 h-3.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
  </svg>
);

const TikTokIcon = ({ className = "w-3.5 h-3.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
  </svg>
);

const YouTubeIcon = ({ className = "w-3.5 h-3.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
  </svg>
);

const WeverseIcon = ({ className = "w-3.5 h-3.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M2.93 6.727 6.633 17.91h.11l2.468-7.47h1.066l2.469 7.47h.109L16.557 6.727h1.151l-4.288 12.546h-1.065L9.93 12.21h-.108l-2.424 7.063H6.333L2.044 6.727zm16.068 0h3.16c.26 0 .474.08.642.242.168.161.252.378.252.65v1.84c0 .272-.084.49-.252.651a.875.875 0 0 1-.642.242h-3.16a.875.875 0 0 1-.642-.242 .903.903 0 0 1-.252-.65V7.618c0-.273.084-.49.252-.651a.875.875 0 0 1 .642-.242z" />
  </svg>
);

const XIcon = ({ className = "w-3.5 h-3.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
  </svg>
);

const SpotifyIcon = ({ className = "w-3.5 h-3.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z" />
  </svg>
);

const platformBadges = [
  { name: "Instagram", icon: InstagramIcon },
  { name: "TikTok", icon: TikTokIcon },
  { name: "YouTube", icon: YouTubeIcon },
  { name: "Weverse", icon: WeverseIcon },
  { name: "X", icon: XIcon },
  { name: "Spotify", icon: SpotifyIcon },
];

export default function CrossPlatformSection() {
  return (
    <section className="relative py-16 sm:py-24 bg-white text-slate-900 overflow-hidden">
      <div className="max-w-screen-xl px-6 mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          {/* Left Column: Image */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-60px" }}
            transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            className="rounded-2xl sm:rounded-3xl overflow-hidden aspect-[4/3] sm:aspect-[16/11] bg-slate-100 shadow-sm border border-slate-200/60"
          >
            <img
              src={heroIdolsImg}
              alt="Every platform. Every post. One place."
              className="w-full h-full object-cover object-center hover:scale-105 transition-transform duration-500"
              onError={(e) => {
                e.currentTarget.src = idolFallback;
              }}
            />
          </motion.div>

          {/* Right Column: Content */}
          <motion.div
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-60px" }}
            transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1], delay: 0.1 }}
            className="text-left"
          >
            <p
              className="text-[11px] sm:text-xs font-bold uppercase tracking-[0.2em] mb-3"
              style={{ color: "#64748b" }}
            >
              CROSS-PLATFORM
            </p>
            <h2
              className="text-3xl sm:text-4xl md:text-[44px] font-extrabold tracking-tight mb-5 leading-[1.12]"
              style={{ color: "#0f172a" }}
            >
              Every platform. Every post.<br />One place.
            </h2>

            {/* Platform Pill Badges */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 mb-6">
              {platformBadges.map((item, idx) => {
                const IconComp = item.icon;
                return (
                  <div
                    key={idx}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-slate-100 border border-slate-200/70 text-xs font-medium text-slate-800 shadow-2xs hover:bg-slate-200/80 transition-colors"
                  >
                    <IconComp className="w-3.5 h-3.5 text-slate-700" />
                    <span>{item.name}</span>
                  </div>
                );
              })}
            </div>

            {/* Description Paragraph */}
            <p
              className="text-sm sm:text-base leading-relaxed mb-8 max-w-xl"
              style={{ color: "#64748b" }}
            >
              Instagram stories. YouTube shorts. Weverse lives. TikTok challenges. X threads. Spotify drops. Stop bouncing between six apps. KpopRadar gives you a single chronological feed tailored to your taste.
            </p>

            {/* CTA Button */}
            <Link
              href="#"
              className="inline-flex items-center justify-center rounded-full bg-[#0f172a] px-7 sm:px-8 py-3 sm:py-3.5 text-sm sm:text-base font-semibold text-white hover:bg-slate-800 transition-all shadow-md active:scale-95"
            >
              Start tracking
            </Link>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
